...
import { createProject, uploadImage } from '@/lib/api/admin'
...
export default function NewProjectPage() {
        ...
        switch (step) {
            ...
            case 3:
                ...
                if (!formData.thumbnail_image) {
                    const proceed = confirm('No thumbnail image added. Continue anyway?')
    ...
    const handleNameChange = (name: string) => {
        setFormData({
            ...formData,
            project_name: name,
            project_slug: generateSlug(name),
            meta_title: name,
            og_title: name
        })
    }
    ...
    const handleRelatedProjects = (projectId: number) => {
        const currentRelated = [...formData.related_projects]
        const index = currentRelated.indexOf(projectId)

        if (index > -1) {
            currentRelated.splice(index, 1)
        } else {
            if (currentRelated.length < 3) {
                currentRelated.push(projectId)
            } else {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const { data, error } = await createProject(cleanedData)
    ...
    return (
        <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-6">
                ...
                <p className="text-gray-600 mt-2">Create a new project step by step</p>
            </div>
